...
void list_traverse(struct List thisList) {
  ...
  struct Node * current = thisList.head;
  while (current != NULL) {
    printf("%s\n", "this is a chunk! Only prints out var value of each node in the list\n");
    printf("%s \n\n\n\n", current->var); //current->var = html, current->val =???
    current = current->next;
  }  /* end of while */
  printf("\n");
};

int list_retrievedlength(struct List thisList) {

  struct Node * current = thisList.head;
  int count = 1;
  int length = 0;
  while (current != NULL) {
    printf("Length of char* stored in node %d is %d\n", count, current->num);
    length += current->num;
    count ++;
    current = current->next;
  }  /* end of while */
